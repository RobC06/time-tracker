...
// Pre-fill form with an existing entry's data (for duplication)
...
function render() {
  ...
  dates.forEach(date => {
    ...
    if (showAllEntries) {
      ...
      const dayMixed = dayBillable > 0 && dayNonBillable > 0;
      const dayAllNonBillable = dayBillable === 0 && dayNonBillable > 0;
      let dayBadges = '';
      if (dayMixed) {
        dayBadges += `<span class="stats-badge billable-badge">B: ${dayBillable.toFixed(2)}h</span>`;
    ...
    Object.keys(clients).forEach(clientKey => {
      ...
      const mixed = clientBillable > 0 && clientNonBillable > 0;
      ...
      clientEntries.forEach(entry => {
        ...
        html += `<button class="copy-btn" title="Duplicate"><svg xmlns="http://www.w3.org/2000/svg" width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><rect x="9" y="9" width="13" height="13" rx="2" ry="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg></button>`;
        html += `<button class="edit-btn" title="Edit"><svg xmlns="http://www.w3.org/2000/svg" width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg></button>`;
        html += `<button class="delete-btn" title="Delete"><svg xmlns="http://www.w3.org/2000/svg" width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/><path d="M10 11v6M14 11v6"/><path d="M9 6V4h6v2"/></svg></button>`;
        html += `</div>`;
        html += `</div>`;
        html += `</div>`;
      });
